...
export function render(data, prop) {

  let index = prop.indexOf(' ');
  const planets = prop.slice(0, index);
  const species = prop.slice(index + 1);


  const container = createElem('div', ['container', 'd-flex', 'flex-wrap', 'justify-content-between', 'gap-5', 'py-5']);
  const episodeCard = createElem('div', ['card', 'mx-auto', 'p-5']);
  episodeCard.style.width = '80%';
  ...
  planetsWrapper.style.width = '40%';
  const speciesWrapper = createElem('div', ['p-4', 'mb-3']);
  speciesWrapper.style.width = '40%';
  ...
  const backLink = createElem('a', ['btn', 'btn-outline-primary', 'p-4'], 'Back to episodes');
